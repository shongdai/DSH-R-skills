...
三种截图各由独立脚本实现，均在 [scripts/](file:///d:/R/.agents/skills/DSH-code-marketing/scripts/) 下：

| 脚本 | 函数 | 用途 | 依赖 |
|------|------|------|------|
| [capture_code.R](file:///d:/R/.agents/skills/DSH-code-marketing/scripts/capture_code.R) | `capture_code(target, script, n=50)` | R 代码语法高亮截图（逐字符扫描+HTML转义，tango 亮色主题） | webshot2 |
| [capture_table.R](file:///d:/R/.agents/skills/DSH-code-marketing/scripts/capture_table.R) | `capture_table(data_path, target, nrows=10)` + `capture_table_batch(csv_dir, ...)` | 通用表格截图：支持 csv/tsv/txt/xlsx，自动格式化数值列，compact/display 双样式，批量截图 | knitr, webshot2, readr |
| [capture_html.R](file:///d:/R/.agents/skills/DSH-code-marketing/scripts/capture_html.R) | `capture_html(html_path, target, stop_at_image=TRUE)` | 使用说明 HTML 长截图（xml2 解析 + Headless Edge 截图，截到第一张图即停） | xml2 |

> [screenshot.R](file:///d:/R/.agents/skills/DSH-code-marketing/scripts/screenshot.R) 与 [collect.R](file:///d:/R/.agents/skills/DSH-code-marketing/scripts/collect.R) 为旧版聚合文件，保留向后兼容。
...
[scripts/collect.R](file:///d:/R/.agents/skills/DSH-code-marketing/scripts/collect.R) 是一键收集脚本，模板存放：

| 模板 | 路径 | 用途 |
|------|------|------|
| 笔记.txt | [templates/note.md](file:///d:/R/.agents/skills/DSH-code-marketing/templates/note.md) | 小红书笔记骨架（标题+正文+标签） |
| 商品设置.txt | [templates/product.md](file:///d:/R/.agents/skills/DSH-code-marketing/templates/product.md) | 闲鱼商品设置（5 段标准） |
...
或者命令行：
```powershell
Rscript "d:\R\.agents\skills\DSH-code-marketing\scripts\collect.R" "d:\R\【P004】桑基气泡图"
```
